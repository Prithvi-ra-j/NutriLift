...
export function SetRow({ set, exerciseType, onEdit }: { set: SetLog; exerciseType: ExerciseType; onEdit: () => void }) {
  return (
    <PressableScale onPress={onEdit} accessibilityRole="button" accessibilityLabel={`Edit set ${set.set_number}`} style={{
      ...
      borderRadius: M3.shape.small, paddingHorizontal: 9, paddingVertical: 6,
      borderWidth: 1, borderColor: set.is_pr === 1 ? M3.colors.warning : M3.colors.outline,
    }}>
      <Text style={{ ...M3.typescale.labelMedium, color: set.is_warmup ? M3.colors.onSurfaceMuted : M3.colors.onSurface }}>{label(set, exerciseType)}</Text>
